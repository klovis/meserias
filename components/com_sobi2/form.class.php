<?php
/**
* @version $Id: form.class.php 5462 2010-08-18 08:25:37Z Sigrid Suski $
...
class sobiForm {
    ...
    function sobiForm( $sobi2Id = 0, $catid )
    {
    	...
    	/* ursprungscode in form.class.ori1.php */
    ...
    function buildFormWithTemplate()
    {
    	...
    	if($config->allowUsingBackground) {
			...
			foreach ( $imageFiles as $file ) {
				if ( sobi2_eregi( "bmp|gif|jpg|png", $file ) ) {
    		...
    		$fields['BackgroundPreview']['field'] = "<script  type=\"text/javascript\">
						...
						document.write('<img src=' + bsimg + ' name=\"backgroundimage\" width=\"50\" height=\"50\" style=\"border-color:#{$config->sobi2BorderColor}; border-width:2px; border-style:solid;\"  alt=\"Preview\" />');
					</script>";
    	}
    ...
    function buildForm()
    {
		...
		if($config->allowUsingIco) {
			// if editing existing item and it has already Icon
    ...
    function isTextarea($field)
    {
    	...
    	if( !$field->wysiwyg && !defined( "_SOBI2_ADMIN" ) ) {	//wenn im Frontend und nicht wysiwyg
