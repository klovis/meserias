<?php
/**
* @version $Id: sobi2.entry.php 5462 2010-08-18 08:25:37Z Sigrid Suski $
    ...
    function showDetails( $sobi2Id, $catid )
    {
		...
		$requestParams =& $config->get_( "requestParams" );
	 	...
	 	if( $sobi2Id ) {
	 		...
	 		if ($config->isset_('sobi_entry'.$sobi2Id)) {	//sollte durch buildHeader erzeugt worden sein
		 		$mySobi =& $config->get_('sobi_entry'.$sobi2Id);
	 		}
	 		else {		
...
//		$ob = ob_get_contents();
    ...
    function save( $catid )
    {
    	...
    	if( count( $newSobi->fees ) ) {
			...
			echo "<div class=\"sobi2AddEditHeader\"><h3>"._SOBI2_FORM_TITLE_ADD_NEW_ENTRY."</h3>";
		...
		$my =& $config->getUser();
 		if( ( $my->id != 0 && $config->allowUserToEditEntry ) || $config->checkPerm() ){
	 		...
	 		if( count( $newSobi->fees ) ) {
				...
				echo "<div class=\"sobi2AddEditHeader\"><h3>"._SOBI2_FORM_TITLE_EDIT_ENTRY."</h3>";
				sobi2Config::import( "payment.class" );
	 			$fees = new payment( $newSobi );
				$fees->showFees();
				echo "</div>";
    ...
    function showSobi( $sobi2Id, $mySobi, $itemData, $waySearchLink, $cid, $fieldsObjects )
    {
		...
		if(!$mySobi->published) {
			...
			$published = $expired || $published ? true : false;
			if( !$published || !$my->id || ( $mySobi->owner != $my->id ) ) {
