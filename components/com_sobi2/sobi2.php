 ...
 */
	...
	function parseParams( &$request )
	{
		if(!strstr( $_SERVER['REQUEST_URI'], 'index.php') && class_exists( 'JSite')) {
			...
			foreach ( $req as $k => $v ) {
				//wenn catid zweimal (passiert wenn Kategorie in Joomla Menue verlinkt und nur Joomla SEF an) dann die 2. nehmen
				if ($k == 'catid' && sobi2Config::request($_REQUEST,'catid',0)) {
					$v = sobi2Config::request($_REQUEST,'catid',0);
				}
