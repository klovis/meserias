<?php
/**
* @version $Id: entry.functions.php 5471 2010-09-01 06:40:13Z Sigrid Suski $
    ...
    function getMyCategories( $mySobi, $string = false, $wholePath = false )
    {
		...
		if( $wholePath && $string ) {
	    	...
	    	foreach ($categories as $category) {
	    		$category->name = stripslashes(stripslashes($category->name));
	    		$category->introtext = stripslashes($category->introtext);
	    		if($category->icon) {
    ...
    function replace( $txt, $key = null )
    {
		...
		if( !$key ) {
    		foreach ( $def as $replace => $with ) {
    			$txt = preg_replace( "/\b{$replace}\b/", $with, $txt );
    		}
		}
		else {
			if( key_exists( $key, $def ) ) {
    				$txt = preg_replace( "/\b{$def[$key]}\b/", $with, $txt );
				//$txt = sobi2_ereg_replace( "[[:<:]]{$def[$key]}[[:>:]]", $with, $txt );
			}
			else {
...
//					$fieldsIgnore = explode(",", $fieldsIgnore );
    ...
    function getThumbshotsOrg( $url, $alt = '', $style = 'border-style:none;', $params = null )
	{
		...
		$days_to_keep = $config->key( "thumshots.org", "days_to_keep", 60 );;
