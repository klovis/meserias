<?php
/**
* @version $Id: helper.class.php 5462 2010-08-18 08:25:37Z Sigrid Suski $
...
class sobi2Helper {
	...
	function meta( $desc, $keys, $cid = 1 )
	{
		...
		if( !isset( $meta[ $cid ] )) {
	    	$query = 'SELECT catid, name, introtext, description FROM #__sobi2_categories WHERE catid = '.$cid;
			$database->setQuery( $query );
			if( !$config->forceLegacy && class_exists( 'JDatabase' ) ) {
				$comMeta = $database->loadObject();
			}
	    	else {
	    		$database->loadObject( $comMeta );
	    	}
	    	$meta[ $cid ] = $comMeta;
		...
		if( $j15 ) {
			if( $config->key( 'general', 'meta_desc_append' ) ) {
			...
			$document->setDescription( str_replace( '  ', ' ', $metaDesc ) );
		}
		else {
			$mainframe->appendMetaTag( 'description', str_replace( '  ', ' ', $metaDesc ) );
		}

		/* first handle the given keys */
		if( strlen( $keys ) ) {
			$metaKeys .= $keys;
		}
		/* if there was something from the category */
		if( count( $meta[ $cid ] ) ) {
			...
			if( $cid > 1 ) {
				...
				$metaKeys .= $config->getSobiStr('Meseriasi, Constructori case, Zugravi, Instalatori, Electricieni');
			}
		}
		$metaKeys = str_replace( '>','> ', $metaKeys );
		$metaKeys = str_replace( array( "\n", "\r", "\t"  ), ',' , $metaKeys );
		if( $j15 ) {
			$cur = null;
			if( $config->key('general', 'meta_keys_append') ) {
				$cur = $document->getMetaData( 'keywords' );
			}
			$metaKeys = trim( $metaKeys );
			$metaKeys = $cur.', '.$metaKeys;
			...
			$document->setMetadata( 'keywords', str_replace( '  ', ' ', $metaKeys ) );
		}
		else {
			$mainframe->appendMetaTag( 'keywords', str_replace( '  ', ' ', $metaKeys ) );
		}
		if( $j15 ) {
			if( !$config->get( 'gen_added',false ) ) {
				$cur = null;
				if( $config->key( 'general', 'generator_append' ) || $config->key( 'general', 'generator_append' ) ) {
					$cur = $document->getGenerator().'. ';
				}
				$gen = 'Si'.'gs'.'iu Online Business Index - '.$config->getSobiStr( $config->componentName );
				if( strlen( $cur ) || strlen( $gen ) ) {
					$gen = $cur . $gen;
					$gen = explode( ' ', $gen );
					if( count( $gen ) ) {
						foreach ( $gen as $i => $p ) {
				...
				$config->set( 'gen_added', true );
			}
		}
		else {
			$mainframe->appendMetaTag( 'generator', '. Si'.'gs'.'iu Online Bu'.'sin'.'ess Index - '.$config->getSobiStr( $config->componentName ) );
		}
	}
	/**
