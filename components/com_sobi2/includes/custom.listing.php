<?php
/**
 * @version $Id: custom.listing.php 5462 2010-08-18 08:25:37Z Sigrid Suski $
...
class sobiCListings
{
	...
	function buildCustomListing ( $sids, $cids, $navMenuUrl, $customHeader = null, $stringForItems = null, $stringForCats = null, $addToPathway = null, $addToSiteTitle = null, $defCid = 0, $pluginTask = null, $itemsOrdering = null, $catsOrdering = null, $pluginsArgs = null, $forceUnpublishedItems = false, $forceUnpublishedCats = false, $skipHeader = false, $skipFooter = false, $templateFunction = null, $itemsInLine = 0, $preventPlugins = false, $forceCache = false )
	{
		...
		if ( $addToSiteTitle ) {
			$pageTitle = array();
			if ( $config->key( 'frontpage', 'browser_title_add_com_name', true ) ) {
				$pageTitle[] = $config->componentName; //Komponentenname
			}
			$pageTitle[] = $addToSiteTitle;
			if ( $j15 ) {
				if ( $config->checkCatItemid( $task ) ) { //wenn direkt verlinkt in Joomla Menue
			...
			if ( $t != - 1 ) { //Seitentitel anzeigen
				if ( ! strlen( $t ) ) { //wenn kein Seitentitel ueber Menue
					if ( count( $pageTitle ) ) {
						...
						$t = implode( $config->key( 'general', 'browser_title_separator', ' - ' ), $pageTitle );
					}
				}
				$mainframe->setPageTitle( html_entity_decode( $t ) );
			}
			else
				$mainframe->setPageTitle( "" ); //Titel l�schen da Hauptroutine bereits was reinschreibt
		}

		//Pathway ausgeben
		if ( ! strlen( $t ) && $addToPathway ) { //wenn nicht verlinkt im Joomla Menue
		...
		if ( is_array( $sids ) && ! empty( $sids ) || ( $pluginOutput && $pluginOutputPosition == "beforeItems" ) || ( is_array( $pluginsOutput ) && ! empty( $pluginsOutput ) ) ) {
			...
			if ( is_array( $sids ) && ! empty( $sids ) ) {
				...
				$cats = sobiCListings::getCats( $sids );
				...
				if ( isset( $results ) && ! empty( $results ) ) {
					...
					if ( isset( $results ) && count( $results ) ) {
						...
						foreach( $results as $result ) {
							...
							if ( isset( $config->sobi2BackgroundImg ) && ! ( empty( $config->sobi2BackgroundImg ) ) )
							...
							if ( ! $my->id && ! $config->allowAnoDetails ) {
								...
								if ( $defCid ) {
									...
									$cid = ( $use_original_catid && isset( $cats[ $result->itemid ] ) && $result->itemid ) ? '&amp;catid=' . $cats[ $result->itemid ] : null;
								}
								$href = "index.php?option=com_sobi2&amp;sobi2Task=sobi2Details{$cid}&amp;sobi2Id={$result->itemid}&amp;Itemid={$config->sobi2Itemid}";
								$href = sobi2Config::sef( $href );
								$onClick = null;
							}
							if ( ( $my->id != 0 && $my->id == $result->owner ) || $config->checkPerm() ) {
								if ( $config->allowUserToEditEntry || $config->checkPerm() ) {
									$ehref = "{$config->liveSite}/index.php?option=com_sobi2&amp;sobi2Task=editSobi&amp;sobi2Id={$result->itemid}&amp;Itemid={$config->sobi2Itemid}{$cid}";
									$editButton = "<input type=\"button\" class=\"button sobi2EditEntryButton\" onclick=\"location.href='{$ehref}'\"  name=\"editEntry\" value=\"" . _SOBI2_LISTING_EDIT_ENTRY_BT . "\"/>";
								}
								if ( $config->allowUserDelete || $config->checkPerm() ) {
									$dhref = "{$config->liveSite}/index.php?option=com_sobi2&amp;sobi2Task=deleteSobi&amp;sobi2Id={$result->itemid}&amp;Itemid={$config->sobi2Itemid}{$cid}";
									$deleteButton = "<input type=\"button\" class=\"button sobi2DelEntryButton\" onclick=\"if(confirm('" . _SOBI2_CONFIRM_DELETE . "') == true) location.href='{$dhref}'\" name=\"editEntry\" value=\"" . _SOBI2_LISTING_DELET_ENTRY_BT . "\"/>";
								}
							}
	...
	function getCats ( $sids )
	{
		if ( count( $sids ) ) {
			$config = & sobi2Config::getInstance();
			$db = $config->getDb();
			$sids = implode( ',', $sids );
			$db->setQuery( "SELECT catid, itemid FROM #__sobi2_cat_items_relations WHERE itemid IN ({$sids}) AND catid > 1 GROUP BY itemid" );
			$results = $db->loadAssocList('itemid');
			if ( $db->getErrorNum() ) {
				trigger_error( "HTML_SOBI::getCats(): DB reports: " . $db->stderr(), E_USER_WARNING );
				return array();
			}
			else {
				$cats = array();
