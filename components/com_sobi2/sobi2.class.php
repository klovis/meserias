<?php
/**
* @version $Id: sobi2.class.php 5466 2010-08-18 14:39:20Z Sigrid Suski $
    ...
    */
    ...
    function getAttributs( $sobi2Id, $data = false )
    {
    	...
    	elseif( $data ) {
    		...
    		//Vorbelegung als ob nix zugelassen und User nicht Owner
			$published = "AND `published` = 1 AND (`publish_down` > '{$now}' OR `publish_down` = '{$config->nullDate}' )";
			$cl = 0;
			
    		switch ( $config->publishedItems )  //dieser wert kann nur 0 oder 4 (4= can_see_expired = 1 in config.class.php) oder 6 oder 7 sein
    		{
    			case 0:
    			default: //nur published und nicht abgelaufen
    				break;
    			case 1: //alles was published ist, auch wenn abgelaufen (Verwendung unbekannt, Plugin?)
    				$published = 'AND `published` = 1 ';
    				$cl = 1;
    				break;
    			case 2: //alles unabh�ngig vom published status (Verwendung unbekannt, Plugin?)
    				$published = null;
    				$cl = 2;
    				break;
    			case 4: //User kann abgelaufene sehen, aber keine unpublished
    				if( $user->id ) {
    				    $cl = 4;
    				    $published = "AND `published` = 1 AND ( (`publish_down` > '{$now}' OR `publish_down` = '{$config->nullDate}' ) OR ( `owner` = {$user->id} ) )";
    				}
    				break;
    			case 5: //wie 0 (Verwendung unbekannt, Plugin?)
    				$cl = 5;
    				$published = "AND ((`published` = 1 AND (`publish_down` > '{$now}' OR `publish_down` = '{$config->nullDate}' )) OR `owner` = {$user->id})";
    				break;
    			case 6:	//user kann unpublished sehen aber keine expired
    				if( $user->id ) {
    					$published = "AND (`publish_down` > '{$now}' OR `publish_down` = '{$config->nullDate}' ) AND (`published` = 1 OR `owner` = {$user->id})";
    				    $cl = 4;
    				}
    				break;
    			case 7:	//user kann unpublished sehen und expired
    				if( $user->id ) {
    				    $published = "AND ((`published` = 1 AND (`publish_down` > '{$now}' OR `publish_down` = '{$config->nullDate}' )) OR ( `owner` = {$user->id} ) )";
    				    $cl = 4;
    				}
    ...
    function saveSobi()
    {
		 ...
		 */
		$this->title = $config->clearSQLinjection( strip_tags( sobi2Config::request( $_POST, 'field_entry_name', null, 0x0002 ) ) );
    	...
    	if( $this->id ) {
	    	 ...
	    	 */
	    	$cids = array();
	    	foreach( $this->newSelectedCats as $index => $catid ) {
				$cids[] = $catid;
	    	...
	    	if( array_sum( $config->catPrices) > 0 ) {
	    		for($i = 0; $i < sizeof($this->newSelectedCats); $i++) {
	    			$cat = $i + 1;
    ...
    function updateSobi()
    {
    	...
    	if( ( $my->id == $this->owner && $config->allowUserToEditEntry ) || $config->checkPerm() ) {
	    	$this->title = $config->clearSQLinjection( strip_tags(sobi2Config::request( $_REQUEST, 'field_entry_name', null, 0x0002 )) );
	    	$this->title = $this->cleanInput( strip_tags( $this->title ) );
	    	$this->getFieldsNames();
	    	$this->getFieldsData();
	    	$this->getExistingCats();
	    	//$this->getSelectedCats();
	    	...
	    	if( intval( sobi2Config::request( $_REQUEST, 'sobi2ImgDelete', 0 ) ) ){
				...
				$success = true;
				$loaded_before = (strlen ($this->image) != 0);
		    	$msgAfterUpdate = $msgAfterUpdate.$this->getImage( $_FILES["sobi2Img"], "img", $success );	//laden
	    		if ($config->allowUsingImg == 2 && $success) {	//wenns was kostet und erfolgreich
	    			if (!$loaded_before) {	//wenn nicht bereits geladen
	    				$this->fees = $this->fees + array( _SOBI2_SAVE_IMAGE_FEES => $config->priceForImg );
		    		}
	    		}
	    		if (!$success)
	    	...
	    	$this->image = ($config->key("frontpage", "default_img") == $this->image)?null:$this->image;
	    	...
	    	/* icon file upload handling */
	       	if( intval(sobi2Config::request( $_REQUEST, 'sobi2IcoDelete', 0 ) ) ){
	    		...
	    		$success = true;
				$loaded_before = (strlen ($this->icon) != 0);
	    		$msgAfterUpdate = $msgAfterUpdate.$this->getImage( $_FILES["sobi2Ico"], "ico", $success );
	    		if ($config->allowUsingIco == 2 && $success) {	//wenns was kostet und erfolgreich
	    			if (!$loaded_before) {	//wenn nicht bereits geladen
	    				$this->fees = $this->fees + array( _SOBI2_SAVE_ICON_FEES => $config->priceForIco );
		    		}
	    		}
    		...
    		$this->icon = ($config->key("frontpage", "default_ico") == $this->icon)?null:$this->icon;
	    	...
	    	if( $publishedField &&  isset( $this->fieldsData[$publishedField] ) ) {
		    	...
		    	$this->published = $this->published ? $config->key( "edit_form", "autopublish_changes", true ) : $this->published;
		    	$this->approved = $this->approved ? $config->key( "edit_form", "autoapprove_changes", true ) : $this->approved;
	    	}
	    	...
	    	foreach( $this->newSelectedCats as $catid) {
				$newRelations[] = $catid;
    ...
    function getFieldsData()
    {
    	...
    	foreach( $this->fields as $field ) {
    		$data = null;
    		/* if could be html */
    		if( $field->fieldType == 2 ) {  //teaxtarea
    		...
    		elseif ( $field->fieldType == 6 ) { //checkbox group
    		...
    		elseif( $field->fieldType == 1 || $field->fieldType == 5 || $field->fieldType == 7 ) {  //input,list,cal
                $req = sobi2Config::request( $_REQUEST, $field->fieldname, null );
    			if( strlen($req) ) {
    				$data = $req;
    				$data = strip_tags( $data );
    				$data = $this->cleanInput( $data );
                    if( $field->fieldType == 1 )    //damit eine Textbox auch den Wert 0 beinhalten kann
    				    $data = $config->clearSQLinjectionLight( $data );
                    else
                        $data = $config->clearSQLinjection( $data );
    		...
    		/* build 'field name <=> field data' array. */
   			...
   			if( $field->is_required && (!strlen($data)) && ( !$field->wysiwyg || $field->fieldType != 2 )) {
				if( $config->debug ) {
    ...
    function getNewSelectedCats()
	{
		$localitati = array(
		    'field_localitati_opt_26' => '5',  //Alba Iulia
			'field_localitati_opt_40' => '5',  //Alexandria
			'field_localitati_opt_1' => '5',  //Arad
			'field_localitati_opt_4' => '6',  //Bacau
			'field_localitati_opt_6' => '7',  //Baia Mare
			'field_localitati_opt_27' => '7',  //Bistrita
			'field_localitati_opt_5' => '8',  //Botosani
			'field_localitati_opt_8' => '9',  //Braila
			'field_localitati_opt_2' => '10', //Brasov
			'field_localitati_opt_3' => '4',  //Bucuresti
			'field_localitati_opt_7' => '11', //Buzau
			'field_localitati_opt_29' => '11', //Calarasi
			'field_localitati_opt_10' => '12',//Cluj-Napoca
			'field_localitati_opt_9' => '13', //Constanta
			'field_localitati_opt_11' => '14',//Craiova
			'field_localitati_opt_12' => '15',//Deva
			'field_localitati_opt_36' => '15',//Dr. Turnu Severin
			'field_localitati_opt_13' => '16',//Focsani
			'field_localitati_opt_14' => '17',//Galati
			'field_localitati_opt_31' => '17',//Giurgiu
			'field_localitati_opt_15' => '18', //Iasi
			'field_localitati_opt_35' => '18', //Ilfov
			'field_localitati_opt_33' => '18', //Miercurea Ciuc
			'field_localitati_opt_16' => '19', //Oradea
			'field_localitati_opt_17' => '20', //Piatra Neamt
			'field_localitati_opt_18' => '21',//Pitesti
			'field_localitati_opt_19' => '22', //Ploiesti
			'field_localitati_opt_42' => '23', //Ramnicu Valcea
			'field_localitati_opt_28' => '23', //Resita
			'field_localitati_opt_25' => '23', //Roman
			'field_localitati_opt_38' => '23', //Satu Mare
			'field_localitati_opt_30' => '23', //Sfantu Gheorghe
			'field_localitati_opt_23' => '23', //Sibiu
			'field_localitati_opt_37' => '23', //Slatina
			'field_localitati_opt_34' => '23', //Slobozia
			'field_localitati_opt_20' => '23', //Targoviste
			'field_localitati_opt_32' => '23', //Targu Jiu
			'field_localitati_opt_21' => '25',  //Targu-Mures
			'field_localitati_opt_22' => '24',  //Timisoara		
            'field_localitati_opt_41' => '24',  //Tulcea	 		
            'field_localitati_opt_24' => '24',  //Vaslui	 	
            'field_localitati_opt_39' => '24'  //Zalau			
		);
			
		$id_categorii = array(
		    'field_localitati_opt_26' => array(    //Alba Iulia 
				'exterioare' => 136,
				'interioare' => 135,
				'constructii'=> 137,
				'instalatii' => 138
				),
		    'field_localitati_opt_40' => array(    //Alexandria
				'exterioare' => 206,
				'interioare' => 205,
				'constructii'=> 207,
				'instalatii' => 208
				),
			'field_localitati_opt_1' => array(    //Arad
				'exterioare' => 31,
				'interioare' => 30,
				'constructii'=> 32,
				'instalatii' => 33
				),
			'field_localitati_opt_4' => array(   //Bacau
				'exterioare' => 35,
				'interioare' => 34,
				'constructii'=> 36,
				'instalatii' => 37
				),	
			'field_localitati_opt_6' => array(   //Baia Mare
				'exterioare' => 39,
				'interioare' => 38,
				'constructii'=> 40,
				'instalatii' => 41
				),	
			'field_localitati_opt_27' => array(   //Bistrita
				'exterioare' => 141,
				'interioare' => 140,
				'constructii'=> 142,
				'instalatii' => 143
				),	
			'field_localitati_opt_5' => array(   //Botosani
				'exterioare' => 43,
				'interioare' => 42,
				'constructii'=> 44,
				'instalatii' => 45
				),	
			'field_localitati_opt_8' => array(  //Braila
				'exterioare' => 47,
				'interioare' => 46,
				'constructii'=> 48,
				'instalatii' => 49
				),	
			'field_localitati_opt_2' => array(    //Brasov
				'exterioare' => 51,
				'interioare' => 50,
				'constructii'=> 52,
				'instalatii' => 53
				),
			'field_localitati_opt_3' => array(    //Bucuresti
				'exterioare' => 27,
				'interioare' => 26,
				'constructii'=> 28,
				'instalatii' => 29
				),	
			'field_localitati_opt_7' => array(  //Buzau
				'exterioare' => 55,
				'interioare' => 54,
				'constructii'=> 56,
				'instalatii' => 57
				),	
			'field_localitati_opt_29' => array(  //Calarasi
				'exterioare' => 151,
				'interioare' => 150,
				'constructii'=> 152,
				'instalatii' => 153
				),		
			'field_localitati_opt_10' => array(  //Cluj Napoca
				'exterioare' => 59,
				'interioare' => 58,
				'constructii'=> 60,
				'instalatii' => 61
				),
			'field_localitati_opt_9' => array(  //Constanta
				'exterioare' => 63,
				'interioare' => 62,
				'constructii'=> 64,
				'instalatii' => 65
				),		
			'field_localitati_opt_11' => array(  //Craiova
				'exterioare' => 67,
				'interioare' => 66,
				'constructii'=> 68,
				'instalatii' => 69
				),
			'field_localitati_opt_12' => array(  //Deva
				'exterioare' => 71,
				'interioare' => 70,
				'constructii'=> 72,
				'instalatii' => 73
				),
			'field_localitati_opt_36' => array(  //Drobeta Turnu Severin
				'exterioare' => 186,
				'interioare' => 185,
				'constructii'=> 187,
				'instalatii' => 188
				),
			'field_localitati_opt_13' => array(  //Focsani
				'exterioare' => 75,
				'interioare' => 74,
				'constructii'=> 76,
				'instalatii' => 77
				),	
			'field_localitati_opt_14' => array(  //Galati
				'exterioare' => 79,
				'interioare' => 78,
				'constructii'=> 80,
				'instalatii' => 81
				),	
			'field_localitati_opt_31' => array(  //Giurgiu
				'exterioare' => 161,
				'interioare' => 160,
				'constructii'=> 162,
				'instalatii' => 163
				),	
			'field_localitati_opt_15' => array(  // Iasi
				'exterioare' => 83,
				'interioare' => 82,
				'constructii'=> 84,
				'instalatii' => 85
				),	
			'field_localitati_opt_35' => array(  //Ilfov
				'exterioare' => 181,
				'interioare' => 180,
				'constructii'=> 182,
				'instalatii' => 183
				),		
			'field_localitati_opt_33' => array(  //Miercurea Ciuc
				'exterioare' => 171,
				'interioare' => 170,
				'constructii'=> 172,
				'instalatii' => 173
				),		
			'field_localitati_opt_16' => array(  //Oradea
				'exterioare' => 87,
				'interioare' => 86,
				'constructii'=> 88,
				'instalatii' => 89
				),		
			'field_localitati_opt_17' => array(  //Piatra Neamt
				'exterioare' => 91,
				'interioare' => 90,
				'constructii'=> 92,
				'instalatii' => 93
				),	
			'field_localitati_opt_18' => array(  //Pitesti
				'exterioare' => 95,
				'interioare' => 94,
				'constructii'=> 96,
				'instalatii' => 97
				),	
			'field_localitati_opt_19' => array(  //Ploiesti
				'exterioare' => 99,
				'interioare' => 98,
				'constructii'=> 100,
				'instalatii' => 101
				),		
			'field_localitati_opt_42' => array(  //Ramnicu Valcea
				'exterioare' => 216,
				'interioare' => 215,
				'constructii'=> 217,
				'instalatii' => 218
				),	
			'field_localitati_opt_28' => array(  //Resita
				'exterioare' => 146,
				'interioare' => 145,
				'constructii'=> 147,
				'instalatii' => 148
				),	
		    'field_localitati_opt_25' => array(  //Roman
				'exterioare' => 131,
				'interioare' => 130,
				'constructii'=> 132,
				'instalatii' => 133
				),	
			'field_localitati_opt_38' => array(  //Satu Mare
				'exterioare' => 196,
				'interioare' => 195,
				'constructii'=> 197,
				'instalatii' => 198
				),	
			'field_localitati_opt_30' => array(  //Sfantu Gheorghe
				'exterioare' => 156,
				'interioare' => 155,
				'constructii'=> 157,
				'instalatii' => 158
				),	
			'field_localitati_opt_23' => array(  //Sibiu
				'exterioare' => 121,
				'interioare' => 120,
				'constructii'=> 122,
				'instalatii' => 123
				),	
		    'field_localitati_opt_37' => array(  //Slatina
				'exterioare' => 191,
				'interioare' => 190,
				'constructii'=> 192,
				'instalatii' => 193
				),	
		    'field_localitati_opt_34' => array(  //Slobozia
				'exterioare' => 176,
				'interioare' => 175,
				'constructii'=> 177,
				'instalatii' => 178
				),						
			'field_localitati_opt_20' => array(  //Targoviste
				'exterioare' => 103,
				'interioare' => 102,
				'constructii'=> 104,
				'instalatii' => 105
				),	
		    'field_localitati_opt_32' => array(  //Targu Jiu
				'exterioare' => 166,
				'interioare' => 165,
				'constructii'=> 167,
				'instalatii' => 168
				),					
			'field_localitati_opt_21' => array(  //Targu Mures
				'exterioare' => 111,
				'interioare' => 110,
				'constructii'=> 112,
				'instalatii' => 113
				),	
			'field_localitati_opt_22' => array(  //Timisoara
				'exterioare' => 107,
				'interioare' => 106,
				'constructii'=> 108,
				'instalatii' => 109
				),
		    'field_localitati_opt_41' => array(  //Tulcea
				'exterioare' => 211,
				'interioare' => 210,
				'constructii'=> 212,
				'instalatii' => 213
				),		
            'field_localitati_opt_24' => array(  //Vaslui
				'exterioare' => 126,
				'interioare' => 125,
				'constructii'=> 127,
				'instalatii' => 128
				),			
            'field_localitati_opt_39' => array(  //Zalau
				'exterioare' => 201,
				'interioare' => 200,
				'constructii'=> 202,
				'instalatii' => 203
		...
		$categorii = array(
			'exterioare' =>  array('field_servicii_opt_1','field_servicii_opt_9', 'field_servicii_opt_12', 'field_servicii_opt_10', 'field_servicii_opt_17', 'field_servicii_opt_19', 'field_servicii_opt_8', 'field_servicii_opt_6', 'field_servicii_opt_13', 'field_servicii_opt_11'),
			'interioare' =>  array('field_servicii_opt_2','field_servicii_opt_3', 'field_servicii_opt_4','field_servicii_opt_17', 'field_servicii_opt_12', 'field_servicii_opt_18', 'field_servicii_opt_19', 'field_servicii_opt_20', 'field_servicii_opt_8', 'field_servicii_opt_16', 'field_servicii_opt_6', 'field_servicii_opt_13', 'field_servicii_opt_14', 'field_servicii_opt_15'),
			'constructii' => array('field_servicii_opt_1', 'field_servicii_opt_5', 'field_servicii_opt_7', 'field_servicii_opt_12', 'field_servicii_opt_10', 'field_servicii_opt_17', 'field_servicii_opt_6', 'field_servicii_opt_11'),
			'instalatii' => array('field_servicii_opt_17', 'field_servicii_opt_18','field_servicii_opt_19', 'field_servicii_opt_20', 'field_servicii_opt_16')
		);
		
		$categorii_selectate = array(false,false,false,false);
		$calitati_selectate = $_POST['field_servicii'];
		$orasul_selectat = $_POST['field_localitati'];
		
			
		foreach($calitati_selectate as $calitate)
		{
			//verific pentru exterioare
			if (!$categorii_selectate[0])
			{
				if (in_array($calitate,$categorii['exterioare']))
				{
					$categorii_selectate[0] = $id_categorii[$orasul_selectat]['exterioare'];
				}
			}
			//verific pentru interioare
			if (!$categorii_selectate[1])
			{
				if (in_array($calitate,$categorii['interioare']))
				{
					$categorii_selectate[1] = $id_categorii[$orasul_selectat]['interioare'];
				}
			}
			//verific pentru constructii
			if (!$categorii_selectate[2])
			{
				if (in_array($calitate,$categorii['constructii']))
				{
					$categorii_selectate[2] = $id_categorii[$orasul_selectat]['constructii'];
				}
			}
			//verific pentru instalatii
			if (!$categorii_selectate[3])
			{
				if (in_array($calitate,$categorii['instalatii']))
				{
					$categorii_selectate[3] = $id_categorii[$orasul_selectat]['instalatii'];
				}
			}
		}
		$this->newSelectedCats = $categorii_selectate;
	}
    ...
    function replaceMarkers( $string )
    {
    	...
    	$entrytitle = stripslashes(stripslashes($this->title));
    	...
    	$string = str_replace( "{title}", $entrytitle, $string );
