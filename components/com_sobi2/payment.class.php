<?php
/**
* @version $Id: payment.class.php 5462 2010-08-18 08:25:37Z Sigrid Suski $
...
class payment {
    ...
    function showFees()
    {
		...
		if($config->useBankTransfer) {
			...
			if($config->useBankTransfer == 1) {
				...
				echo "\n\t\t\t\t<p class=\"sobi2PaymentMethodHeader\" >"._SOBI2_PAY_WITH_BANK."</p>"._SOBI2_PAY_BANK_DATA_JS_HEADER;
				$config->bankData = str_replace(array( '\r', '\n' ), array( "\r", "\n" ),$config->bankData);
				$config->bankData = stripslashes(stripslashes($config->bankData));
				echo " <br/><br/>{$config->bankData}<br/>";
				echo _SOBI2_PAY_BANK_DATA_JS_TITLE." ".$config->payTitle." "._SOBI2_NUMBER_H." ".$this->sobi2Id."<br/>";
		...
		?>
			...
			<tr>
				<td>
				<p class="sobi2PaymentMethodHeader"><?php echo _SOBI2_PAY_WITH_PAYPAL; ?></p>
    ...
    function mailFees()
    {
		...
		/*
		 * replacing selected option marker
		 */
		 ...
		 * Send email to user
		 */
		$config->mailFooter = str_replace(array( '\r', '\n' ), array( "\r", "\n" ),$config->mailFooter);
		$config->mailFooter = stripslashes(stripslashes($config->mailFooter));
    ...
    function renew( $sid, $save = false )
    {
		...
		echo "<div class=\"sobi2RenewHeader\"><h3>"._SOBI2_RENEW_HEADER."</h3>";
		if( !$save ) {
